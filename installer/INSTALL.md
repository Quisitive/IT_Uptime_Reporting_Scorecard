# Installing the IT Scorecard Collector (Windows Service)
...
```powershell
.\install.ps1 -Silent -InstallDir "C:\Program Files\ITScorecard" `
...
## What it does
- Copies the app to the install directory (default `C:\Program Files\ITScorecard`).
- Places the bundled `runtime\node.exe` (or downloads Node 22 if not bundled).
- Registers a Windows Service **"IT Scorecard Collector"** via the WinSW wrapper
...
| Action | Command |
|---|---|
| Start / stop / restart | `services.msc` → "IT Scorecard Collector", or `service\ITScorecard.exe restart` |
| View logs | `<InstallDir>\service\logs\ITScorecard.out.log` (and `.err.log`) |
...
```powershell
cd scorecard-collector\installer
...
```
Output: `dist\ITScorecard-Installer-<version>.zip`. Ship that zip to operators.
